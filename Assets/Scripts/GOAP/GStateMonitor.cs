...
public class GStateMonitor : MonoBehaviour
{
    ...
    public GResource resourcePrefab;
    public string queueName;
    public string worldState;
    public GActionData actionData;
    ...
    void LateUpdate()
    {
        ...
        if (stateFound)
        {
            ...
            if (stateStrength <= 0)
            {
                ...
                GResource p = Instantiate<GResource>(resourcePrefab, location, resourcePrefab.transform.rotation);
                stateFound = false;
                stateStrength = initialStrength;
                beliefs.RemoveState(state);
                GWorld.AddResource(p);
